...
class ReminderManager {
    ...
    private func reminderBody(event: EKEvent, minutesBefore: Int, profile: UserProfile) -> String {
        ...
        if lower.contains("interview") {
            ...
            return "\(title) starts in \(timeStr). You're interviewing for \(role) — take a breath, you've got this. Open MacOverlay to get real-time coaching."
        } else if lower.contains("standup") || lower.contains("stand-up") || lower.contains("sync") {
            return "\(title) in \(timeStr). Open MacOverlay to capture action items and decisions in real time."
        } else if lower.contains("call") || lower.contains("meeting") || lower.contains("review") {
            return "\(title) in \(timeStr). Switch MacOverlay to Meeting mode to get summaries and action items."
        } else {
            return "\(title) starts in \(timeStr). MacOverlay is ready to assist."
        }
    }
}
